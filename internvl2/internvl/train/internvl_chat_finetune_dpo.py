...
from internvl.trl.trainer.dpo_trainer import DPOTrainer
from internvl.trl.trainer.utils import DPODataCollatorWithPadding
import copy
...
class DataTrainingArguments:
    ...
    data_path: Optional[str] = field(
        default=None,
        metadata={'help': 'The path of the dpo jsonl file.'},
    )
    image_folder: Optional[str] = field(
        default=None,
        metadata={'help': 'The path of the image folder.'},
    )
...
class DPODataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        self.list_data_dict = load_data(data_args)
        self.tokenizer = tokenizer
        self.root = data_args.image_folder
        self.is_train = is_train
        self.pad2square = data_args.pad2square
        self.normalize_type = data_args.normalize_type
        self.min_dynamic_patch = data_args.min_dynamic_patch
        self.max_dynamic_patch = data_args.max_dynamic_patch
        self.use_thumbnail = data_args.use_thumbnail
        self.dynamic_image_size = data_args.dynamic_image_size
        self.num_image_token = num_image_token
        self.group_by_length = group_by_length

        self.template_name = data_args.conv_style
        self.image_size = data_args.force_image_size
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        chosen_data_item = {'image': data_dict['image_path'], 'conversations': make_conv(data_dict['prompt'], data_dict['chosen'])}
        rejected_data_item = {'image': data_dict['image_path'], 'conversations': make_conv(data_dict['prompt'], data_dict['rejected'])}

        chosen_ret = self.multi_modal_get_item(chosen_data_item)
        rejected_ret = self.multi_modal_get_item(rejected_data_item)

        padded_batch = {
            'chosen_input_ids': chosen_ret['input_ids'].unsqueeze(0),
            'chosen_labels': chosen_ret['labels'].unsqueeze(0),
            'rejected_input_ids': rejected_ret['input_ids'].unsqueeze(0),
            'rejected_labels': rejected_ret['labels'].unsqueeze(0),
            'chosen_attention_mask': chosen_ret['attention_mask'].unsqueeze(0),
            'rejected_attention_mask': rejected_ret['attention_mask'].unsqueeze(0),
            'pixel_values': chosen_ret['pixel_values'],
            'image_flags': chosen_ret['image_flags']
        }
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        model = InternVLChatModel(config=config)
    else:
